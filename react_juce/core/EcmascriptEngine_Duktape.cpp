    ...
    struct EcmascriptEngine::Pimpl : private juce::Timer
    {
        ...
        void pushVarToDukStack (std::shared_ptr<duk_context> ctx, const juce::var& v, bool persistNativeFunctions = false)
        {
            ...
            if (v.isObject())
            {
				...
				/** Added by Isaac, may be incorrect.
						This jassert means you're adding an object that is a ReferenceCountedObject,
						but not a DynamicObject. Only DynamicObjects can be accessed from Javascript.
					Also, allowing this function to fail will cause a later failure, as the
						duk stack will not be large enough to support the later attempt to
						normalize the index & push the object. (This function should really return a boolean.)
					If all this is your intended use case, feel free to remove the jassert.
				 */
				else jassertfalse;
