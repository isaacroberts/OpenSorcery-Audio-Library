...
//==============================================================================
/**
	Describes one of the sounds that a RTSynth can play.

	A synthesiser can contain one or more sounds, and a sound can choose which
	midi notes and channels can trigger it.

	The RTSynthSound is a passive class that just describes what the sound is -
	the actual audio rendering for a sound is done by a RTSynthVoice. This allows
	more than one RTSynthVoice to play the same sound at the same time.

	@see RTSynth, RTSynthVoice

	@tags{Audio}
*/
class JUCE_API  RTSynthSound    : public ReferenceCountedObject
{
protected:
	//==============================================================================
	RTSynthSound();
...
public:
	/** Destructor. */
	~RTSynthSound() override;

	//==============================================================================
	/** Returns true if this sound should be played when a given midi note is pressed.

		The RTSynth will use this information when deciding which sounds to trigger
		for a given note.
	*/
	...
	*/
	...
	using Ptr = ReferenceCountedObjectPtr<RTSynthSound>;


private:
	//==============================================================================
	JUCE_LEAK_DETECTOR (RTSynthSound)
};


//==============================================================================
/**
	Represents a voice that a RTSynth can use to play a RTSynthSound.

	A voice plays a single sound at a time, and a synthesiser holds an array of
	voices so that it can play polyphonically.

	@see RTSynth, RTSynthSound

	@tags{Audio}
*/
class JUCE_API  RTSynthVoice
{
public:
	//==============================================================================
	/** Creates a voice. */
	...
	*/
	RTSynthSound::Ptr getCurrentlyPlayingSound() const noexcept     { return currentlyPlayingSound; }
	...
	*/
	virtual bool canPlaySound (RTSynthSound*) = 0;
	...
	*/
	virtual void renderNextBlock (ContextR& c,
								  int startSample,
								  int numSamples) = 0;

	virtual void renderTaper(ContextR&, int startIx, int numSamples, int taperIx, int totalTaperLength) = 0;
	...
	*/
	...
	bool wasStartedBefore (const RTSynthVoice& other) const noexcept;
...
protected:
	...
	*/
	virtual void clearCurrentNote();

//	bool hasRendered() { return firstRender;}
protected:
	//==============================================================================
	friend class RTSynth;
	friend class MultiLegatoSynth;
	
	double currentSampleRate = 44100.0;
	int currentlyPlayingNote = -1, currentPlayingMidiChannel = 0;
	uint32 noteOnTime = 0;
	RTSynthSound::Ptr currentlyPlayingSound;
	bool keyIsDown = false, sustainPedalDown = false, sostenutoPedalDown = false;
//	bool firstRender=false;
	
	JUCE_LEAK_DETECTOR (RTSynthVoice)
};
...
/**
	...
	To create a synthesiser, you'll need to create a subclass of RTSynthSound
	to describe each sound available to your synth, and a subclass of RTSynthVoice
...
public:
	...
	*/
	...
	RTSynthVoice* getVoice (int index) const;
	...
	/** Adds a new voice to the synth.
		...
		voice.
	*/
	RTSynthVoice* addVoice (RTSynthVoice* newVoice);
	...
	RTSynthSound::Ptr getSound (int index) const noexcept       { return sounds[index]; }
	...
	*/
	RTSynthSound* addSound (const RTSynthSound::Ptr& newSound);
	...
	*/
	virtual void noteOn (int midiChannel,
						 int midiNoteNumber,
						 float velocity, ContextR& c, int s, int n);
...
//	virtual void setCurrentPlaybackSampleRate (double sampleRate);
	...
	virtual void prepare(Spec& spec);
	...
	*/
//	void renderNextBlock(AudioBuffer<float>& buffer, const MidiBuffer& midi, int startSample, int numSamples, BlockStorage& blockStorage);
	void process (ContextR& c, const MidiBuffer&);
	void process (ContextNR& cnr, const MidiBuffer& m) {
		if (cnr.isBypassed())
		{
			cnr.bypassProcess();
		}
	...
	bool processTail (ContextR& c, const MidiBuffer& midiData);
	bool processTail (ContextNR& cnr, const MidiBuffer& midiData) {
		if (!cnr.isBypassed())
		{
			ContextR c (cnr);
			return processTail(c, midiData);
		}
		else return false;
	}
	
	void processSilence(int numSamples, const MidiBuffer& midiData);
	void processSilence(ContextR& c, const MidiBuffer& m) {
		processSilence(c.getNumSamples(), m);
		if (hardStopTaper)
			hardStopTaper->renderTaper(c.getOutput());
	}
	void processSilence(ContextNR& c, const MidiBuffer& m) {
		processSilence(c.getNumSamples(), m);
		if (hardStopTaper)
			hardStopTaper->renderTaper(c.getOutput());
	}
...
protected:
	...
	OwnedArray<RTSynthVoice> voices;
	ReferenceCountedArray<RTSynthSound> sounds;
	...
	*/
	virtual void renderVoices (ContextR& c,
							   int startSample, int numSamples);

	void gatherTaper(RTSynthVoice*, ContextR& c, int s, int n);
	...
	*/
	virtual RTSynthVoice* findFreeVoice (RTSynthSound* soundToPlay,
											 int midiChannel,
											 int midiNoteNumber,
	 ...
	 */
	virtual RTSynthVoice* findVoiceToSteal (RTSynthSound* soundToPlay,
												int midiChannel,
	...
	*/
	virtual void startVoice (RTSynthVoice* voice,
					 RTSynthSound* sound,
					 int midiChannel,
					 int midiNoteNumber,
					 float velocity);
	...
	/** Stops a given voice.
		...
		RTSynthVoice::stopNote(), and has some assertions to sanity-check a few things.
	*/
	void stopVoice (RTSynthVoice*, float velocity, bool allowTailOff);

	/** Can be overridden to do custom handling of incoming midi events. */
	virtual void handleMidiEvent (const MidiMessage&, ContextR& c, int s, int n);
	virtual void handleMidiEvent_NoOns (const MidiMessage&);

public:
	void createHardStopTaper(int blockLength) {
		hardStopTaper = std::make_shared<HardStopTaper>(blockLength);
	}
	void setHardStopTaper(std::shared_ptr<HardStopTaper> set) {
		hardStopTaper = set;
	}
	std::shared_ptr<HardStopTaper> getHardStopTaper() { return hardStopTaper; }
	void shareHardStopTaper(RTSynth* other) {
		hardStopTaper = other->hardStopTaper;
	}
...
protected:
	...
	std::shared_ptr<HardStopTaper> hardStopTaper=nullptr;
	
	BigInteger sustainPedalsDown;

	JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (RTSynth)
};
