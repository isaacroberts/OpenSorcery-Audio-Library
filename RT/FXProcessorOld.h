...
#ifdef OLD_PRESETS
#include "RTGraphics/Presets/Presets.h"
#else
 ...
 */
	...
	virtual AudioProcessorParameter* getBypassParameter() const override = 0;
...
public:
		...
		bypass(false)
	{
		...
		{
			instrData = new InstrData();
			processorChain->setIntrData(instrData);
		}
	}
	//==============================================================================
private:
...
//		}
//	}
//#error "this isn't supposed to be here"
	void protectYourEars(AudioBuffer<float>& buff)
	{
		int N = buff.getNumSamples();
		for (int c=0; c < buff.getNumChannels(); ++c)
