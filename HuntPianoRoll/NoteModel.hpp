...
public:
	...
	PRESequence(int ticksPerTimeSig) :events(), ticksPerSig(ticksPerTimeSig) {}
	~PRESequence() {}
	PRESequence(const PRESequence& other)= delete; // copy constructor
	PRESequence(PRESequence&& other) noexcept; // move constructor
	PRESequence& operator=(const PRESequence& other) = delete; //copy assignment
	PRESequence& operator=(PRESequence&& other) noexcept; // move assignment

	void autobound();
	void print ();
	
	juce::MidiMessageSequence getMidiMessageSequence(float tickTimeFactor);
	static PRESequence fromMidiMessageSequence(juce::MidiMessageSequence& s, float timeFactor, int ticks);
	
	void render(juce::MidiBuffer& buff, int numSamples, double sampleRate, float timeFactor);
	void render(juce::MidiBuffer& buff, int numSamples, double sampleRate, float timeFactor, float playLength);

	void play();
	void loop();
	
	friend class NoteGridComponent;
private:
